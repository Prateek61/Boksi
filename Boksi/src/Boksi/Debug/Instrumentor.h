...
namespace Boksi
{
	...
	public:
		...
		void BeginSession(const std::string& name, const std::string& filepath = "results.json")
		{
			...
			if (m_CurrentSession)
			{
				...
				{
					BK_CORE_ERROR("Instrumentor::BeginSession('{0}') when session '{1}' already open.", name,
	...
	private:
		// Bool used to enable/disable profiling at runtime.
		// Different from BK_PROFILE which is used to enable profiling at compile time.
		bool m_ProfilingEnabled = true;

	private:
		Instrumentor()
			: m_ProfilingEnabled(true), m_CurrentSession(nullptr)
		{
		}
	...
	public:
		...
		void Stop()
		{
			auto& instrumenter = Instrumentor::Get();

			if (Instrumentor::IsProfilingEnabled())
			{
